...
**現在は未設定**。クラサポ会計専用の AWS アカウントが未決定のためで、決まるまでスクリプトは何も実行しない。

開発機にある既存プロファイルはすべて**他プロジェクトのもの**で、本プロジェクトでは使わない。

| プロファイル | アカウント | 備考 |
|---|---|---|
| `Stay-Sync` | 579617311268 | **旧 `[default]`**。指定漏れで引かれないようリネーム済み |
| `en-scheduler` | 266913442918 | |
| `ehd-dev` | 304838115932 | |
| `kpe` | 497675597696 | |
| `kidsplus-work` | 924955165193 | |
| `kidsplus-link` | 459119163530 | |
| `mado-kit` | 519741283869 | |

> `[default]` セクションは存在しない。プロファイルを指定せずに `aws` を叩くと、黙って他プロジェクトのアカウントを触るのではなく「Unable to locate credentials」で失敗する。これは意図した状態。
