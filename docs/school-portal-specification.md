...
2. **全クラブ反映**: 保存時に `classapo_categories` へ同期（`schoolCommonCategories.ts`）
...
2. **全クラブ反映**: 保存時に `classapo_account_titles` へ同期（`schoolCommonAccountTitles.ts`）。クラブが入力済みの期首残高は上書きしない
