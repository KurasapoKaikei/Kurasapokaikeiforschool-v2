...
| **設定** | 共通カテゴリー（全クラブへ自動反映）・共通科目・担当者のマスタ設定。監査フロー有効時は監査運用設定も利用可能 |
...
| 2 | 監査 | `/school/audit` | リンク | 常時 |
| 3 | 繰越 | `/school/rollover` | リンク | 常時 |
| 4 | クラブ管理 | `/school/clubs` | 親（アコーディオン） | 常時 |
| 5 | 監査人管理 | `/school/clubs/auditors` | 親（アコーディオン） | 監査フロー有効時のみ |
| 6 | メッセージBOX | `/school/messages` | 親（アコーディオン） | 常時 |
| 7 | 契約状況 | `/school/contract` | リンク | 常時 |
| 8 | 設定 | `/school/settings` | 親（アコーディオン） | 常時 |
| 9 | 操作ガイド | `/school/guide` | リンク | 常時 |
...
`/school` 最上部に `SchoolAuditPeriodStatusCard` を表示する（監査期間中／監査期間外。期間中は提出区分・両期限）。設定・通知・解除操作はサイドメニュー「監査」のみ。

#### ポータルトップの監査進捗サマリー

その下の `SchoolAuditProgressSummary` は監査ステータス（未提出・監査中・差戻・承認済）の件数集計のみを表示する。補助説明文は「全クラブの監査ステータス」。各ステータスカードにはバッジ・件数・進捗バーのみを表示し、補足説明文（「未提出・ロックなし」等）は付けない。

#### 監査・繰越（サイドメニュー）

| 画面 | URL | 内容 |
|------|-----|------|
| 監査 | `/school/audit` | 上部に現在のステータス（全幅）。その下に提出区分・決算データ提出期限・監査完了期限の指定、「全クラブ・全監査人に通知」（クラブ／監査人のメッセージBOXへ各1通）、その下に「監査期間を解除」 |
| 繰越 | `/school/rollover` | 年度繰越処理 |

（旧ポータルトップの「決算・年度繰越」ブロックは上記へ分離済み）
...
**監査進捗サマリー（§4.5）— 2026-07-17**

1. **補助説明文**: 「全クラブの監査ステータス」に変更
2. **ステータスカード補足説明の削除**: 「未提出・ロックなし」「監査中かつ未承認」「監査人差戻し中」「監査人承認・完全ロック」を非表示

**共通カテゴリー設定（§2）— 2026-07-17**

1. **実装**: `/school/settings/category` で共通カテゴリーの追加・編集・削除・並び替え
2. **全クラブ反映**: 保存時に登録済み全クラブへ `classapo_categories__{clubId}` として同期（`schoolCommonCategories.ts`。2026-08-04〜クラブスコープ化）
3. **クラブ側**: 学校登録カテゴリーは編集・削除不可（「学校共通」バッジ）。学校保存後は追加UIも非表示
4. **削除制限**: 選択年度にいずれかのクラブで仕訳があれば削除不可。名称編集はいつでも可（当年度仕訳のみ波及）
5. **クラブ追加権限スイッチ**: 「クラブごとにカテゴリーの追加権限を与える」許可する／許可しない（既定: 許可しない）。説明は？マークで表示。選択年度にクラブ独自カテゴリーが1件でもあれば許可しないへ変更不可（0件で変更可）

**共通科目設定（§2）— 2026-07-17**

1. **実装**: `/school/settings/account-titles` で共通科目の追加・編集・削除・並び替え・カテゴリー紐付け（**期首残高は入力しない**）
2. **全クラブ反映**: 保存時に登録済み全クラブへ `classapo_account_titles__{clubId}` として同期（`schoolCommonAccountTitles.ts`。2026-08-04〜クラブスコープ化）。クラブが入力済みの期首残高は上書きしない
3. **クラブ側**: 学校共通科目（現金・預金含む）は名称・カテゴリー・削除不可（右端「学校共通」バッジ）。**初期残高（円）は一覧の入力枠から全科目で入力**（ご利用初年度のみ・列見出し中央寄せ）。現金・預金のクラブ独自科目は権限オフでも常時追加・編集可。収入・支出の独自追加は権限 ON のときのみ
4. **削除制限**: 選択年度にいずれかのクラブで仕訳があれば削除不可。名称編集はいつでも可（当年度仕訳のみ波及）
5. **クラブ追加権限スイッチ**: 「クラブごとに科目の追加権限を与える」許可する／許可しない（既定: 許可しない）。現金・預金のクラブ独自科目は常時追加・編集可のため対象外。選択年度にクラブ独自の収入・支出科目が1件でもあれば許可しないへ変更不可（0件で変更可）
6. **UI**: 学校ネイビー `#172554`（共通カテゴリー設定と同テーマ）。クラブ側は水色 `#77B8DA`
