...
|----|------------|------------------------------|------|
| 左 | 現在の残高 | `#E66A84`（ピンク） | 科目が多い場合はカード内のみスクロール |
| 中央 | メッセージBOX | `#4A90E2`（青） | 本仕様書 §3 参照 |
| 右（上） | 現在の部員数 | `#9D8CC3`（パープル） | **上段**に配置 |
| 右（下） | 重要：未処理・エラー通知 | `#FF0000`（赤） | **下段**に配置。証憑なし支出件数など |

右カラムは縦に 2 段（部員数 → エラー通知）とし、部員数ブロックの flex 比率をやや大きく（`flex-[1.15]`）して視認性を確保する。

### 2.3 年度切替・作業者表示（`ClubPortalYearBar`）

| 項目 | 仕様 |
|------|------|
| 配置 | トップ画面・メッセージBOX専用ページ・決算ページなどクラブ主要画面の **コンテンツ直上** |
| 左側 | 「年度切替:」ラベル + 年度ボタン（2024 / 2025 / 2026 年度） |
| 右側 | **現在の作業者: [氏名または 未選択]** |
| タイポグラフィ | 年度ラベル・作業者行とも **`text-xs`**、色 **`#6B7280`**（ラベル）/ **`#374151`**（作業者名）で統一しコンパクトに表示 |
| 作業者のデータ源 | `localStorage` の担当者設定（`getCurrentOperator()`）。未設定時は「未選択」 |
| 将来 | 注記「※チェックインは今後対応（表示のみ）」— 現時点は **表示のみ**、操作不可 |

---

## 3. メッセージBOX（トップ画面・中央カード）
...
- [ ] 右カラムで「部員数」が上、「エラー通知」が下
- [ ] `ClubPortalYearBar` で年度切替と作業者が同一行・同一文字サイズ感で表示される
- [ ] トップ中央カードの右上に「一覧はこちら ➔」があり、`/club/messages` へ遷移する
