...
4.2 共通ヘッダの二段構成  
一段目：クラブ名・会計期間の下、機能の大分類を示す色付き帯。予実管理配下では紺帯に白抜きで「予実管理」と示す。二段目：白地の帯。左端に紺色の縦アクセント。現在ページの名称（例：予算書、前年度比）を紺色の太字で示し、補足文は小さなグレー。二重の飾り枠は用いない。必要に応じ、さらに下にタブ、表本体を積み上げ、集計帳と同じ上から下の読み順にする。
