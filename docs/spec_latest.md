...
| クラブ画面 | `/club/settings/category` — 学校登録カテゴリーは編集・削除不可（追加UIも非表示）。`fromSchool` フラグ／「学校共通」バッジ |
...
共通科目設定（`/school/settings/account-titles`）は別途実装予定。
...
4. **クラブポータル・現在の残高** — 通常時は現金預金のみ表示。資産・負債・実質残高は繰延で非ゼロ残高がある場合のみ表示
