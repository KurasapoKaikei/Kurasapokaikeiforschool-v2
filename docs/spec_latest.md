...
**入力順**: 処理区分 → 日付 → 収入/支出 → カテゴリー → 科目 → 繰延科目 →（精算時: 精算項目・決済口座）→ 金額 → メモ

| 項目 | 仕様 |
|------|------|
| 処理区分 | **計上** / **精算**（ラジオボタン）。計上選択時の日付は **期末日（3/31）** を自動設定（ヘッダー選択年度。編集不可）。精算は本日 |
| 収入 / 支出 | ラジオで選択。科目候補を収入科目または支出科目に絞り込む（科目名の前に「収入—」等は付けない） |
| カテゴリー | クラブのカテゴリーマスタから選択（科目の絞り込みに使用） |
| 科目 | 選択した収入/支出＋カテゴリーに属する科目名のみ表示 |
| 繰延科目 | ラジオ一覧（科目名＋説明を横並び）。**収入**選択時は仮払金・未払金をロック（薄グレー）。**支出**選択時は未収入金・預り金をロック |
| 相手先 | **廃止**（入力欄なし） |

| 繰延科目 | 種別 | 説明 |
|----------|------|------|
| 未収入金 | 資産 | あとで入ってくるお金（集金予定の部費など） |
| 仮払金 | 資産 | あとで領収書やおつりが戻ってくる（大会の現地出費などで事前渡したお金） |
| 預り金 | 負債 | あとで支払う / 返す（他から一時的に預かっているお金） |
| 未払金 | 負債 | あとで払わなければいけないお金（購入済みの備品代など後払い分） |
...
| 項目 | 仕様 |
|------|------|
| 検索条件 | 資産・負債科目（すべて／未収入金／仮払金／預り金／未払金） |
| 表示列 | 日付、カテゴリー、科目、計上額、精算額、残高、メモ、編集、削除 |
