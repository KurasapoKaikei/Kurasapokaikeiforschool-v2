...
export function ClubPortalHeader() {
  ...
  const clubName = activeClub?.name ?? userInfo.organizationName ?? mockUserInfo.organizationName
    ...
    <header className="sticky top-0 z-50 shadow-sm" style={{ backgroundColor: CLUB_BRAND_PINK }}>
      <div className="flex h-14 items-center justify-between gap-4 px-6">
        ...
        <div className="flex shrink-0 items-center gap-3 sm:gap-4">
          <span className="hidden text-sm text-white/95 sm:inline whitespace-nowrap">
            会計期間：{fiscalPeriod}
          </span>
          <span className="text-xs text-white/95 sm:hidden whitespace-nowrap">
            {fiscalPeriod}
          </span>
          <button
            type="button"
